...
// Enhanced function to set user context with Clerk JWT
async function setUserContextWithJWT(userId, clerkToken = null) {
  try {
    if (!userId) {
      await supabase.rpc('set_user_context', { user_id: null });
      return;
    ...
    // If we have a Clerk token, we could potentially use it for additional validation
    // For now, we'll use the user ID directly
    const { error } = await supabase.rpc('set_user_context', { user_id: userId });
    if (error) {
      console.error('Error setting user context with JWT:', error);
    }
  } catch (error) {
    console.error('Error setting user context with JWT:', error);
  }
