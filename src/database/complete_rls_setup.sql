...
CREATE POLICY "Users can update own data" ON users
    FOR UPDATE USING (current_setting('app.current_user_id', true)::uuid = id);

-- Threads table policies
CREATE POLICY "Users can create threads" ON threads
    FOR INSERT WITH CHECK (current_setting('app.current_user_id', true)::uuid = user_id);
...
CREATE POLICY "Users can delete own threads" ON threads
    FOR DELETE USING (current_setting('app.current_user_id', true)::uuid = user_id);
...
CREATE POLICY "Users can create messages" ON messages
    FOR INSERT WITH CHECK (
        EXISTS (
            ...
            AND threads.user_id = current_setting('app.current_user_id', true)::uuid
        )
    );
